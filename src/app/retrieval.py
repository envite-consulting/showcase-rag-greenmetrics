...
def _bm25_rerank(question: str, docs: list[str], metas: list[dict[str, Any]]) -> tuple[list[str], list[dict[str, Any]]]:
    ...
    # BM25: higher score means better match.
    order = sorted(range(len(docs)), key=lambda i: scores[i], reverse=True)
    logger.info(f"New text segment order after BM25 re-ranking: {order}.")
...
def retrieve(cfg: Config, question: str):
    ...
    # OPTION 1) Metadata filter.
    if cfg.metadata_filter and chunking_is_structure:
        hits = _apply_metadata_filter(hits)

    # OPTION 2) Distance filter; only active for valid threshold values.
    threshold = float(cfg.similarity_threshold)
    if 0.0 < threshold <= 2.0:
        hits = [h for h in hits if h["dist"] <= threshold]

    # If the filters remove every hit.
    if not hits:
        return [], []

    # Load document segments for re-ranking and metadata enhancement.
    ...
    for rank, h in enumerate(hits, start=1):
        ...
        try:
            ...
            logger.warning(f"Chunk could not be loaded: {src} ({e})")
            continue
    ...
    # If no document segments could be loaded.
    if not docs:
        return [], []

    # OPTION 3) BM25 document re-ranking.
    if cfg.post_bm25_rerank:
        docs, out_metas = _bm25_rerank(question, docs, out_metas)

    # OPTION 4) Add metadata as document headers.
    if cfg.metadata_enhancement and chunking_is_structure:
        docs = _enhance_context(docs, out_metas)

    max_top_k = cfg.top_k
    logger.info(f"Retriever returned {len(docs)} text segments.\n"
                f"Top-k = {max_top_k if max_top_k < len(docs) else len(docs)} text segments are used as context.")
